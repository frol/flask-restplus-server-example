...
def test_submission_streamlined(flask_app_client, regular_user, db):
    ...
    try:
        ...
        test_major_type = SubmissionMajorType.test
        project_root = join(config.TestingConfig.PROJECT_ROOT, 'tests', 'submissions', 'test-000')
        test_image_list = ['zebra.jpg', 'fluke.jpg']
        files=[
            _upload_content(join(project_root, filename))
        ...
        for filename in test_image_list:
            assert filecmp.cmp(join(repo.working_tree_dir, filename), join(project_root, filename))
